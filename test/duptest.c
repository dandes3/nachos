...
main()
{
  ...
  char buffer[20], buffer2[2], onechar, onechar2;
  OpenFileId sharedfile, duped;
  char *args[2];
  ...
  prints("Duping file descriptor\n");
  duped = Dup(sharedfile);
  
  prints("Dup returned descriptor ", ConsoleOutput);
  printd((int)duped, ConsoleOutput);
  prints("\n", ConsoleOutput);
  
  numbytes = Read(buffer2, 1, sharedfile);
  prints("PARENT read ", ConsoleOutput);
  printd(numbytes, ConsoleOutput);
  prints(" bytes from duped fd\n", ConsoleOutput);
  
  prints("Data from the read was: <", ConsoleOutput);
  Write(buffer2, 1, ConsoleOutput);
  prints(">\n", ConsoleOutput);
  ...
  numbytes = Read(&onechar2, 1, sharedfile);
  prints("PARENT read from closed file returned ", ConsoleOutput);
  printd(numbytes, ConsoleOutput);
  prints("\n", ConsoleOutput);

  numbytes = Read(&onechar2, 1, duped);
  prints("PARENT read from dup file returned ", ConsoleOutput);
  printd(numbytes, ConsoleOutput);
  prints("\n", ConsoleOutput);
